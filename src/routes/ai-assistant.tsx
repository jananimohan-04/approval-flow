...
function AiAssistantPage() {
    ...
    return (
        <AppShell
            ...
            actions={
                <Button variant="outline" onClick={() => setMessages([])} disabled={loading || messages.length === 0}>
        ...
        >
            <div className="flex flex-col h-[calc(100vh-12rem)] border rounded-xl overflow-hidden bg-background shadow-sm">
                ...
                {messages.length === 0 ? (
                    <div className="flex-1 overflow-y-auto p-4 sm:p-6 flex flex-col items-center justify-center text-center bg-card">
                        ...
                        <p className="text-muted-foreground max-w-sm mb-8">
                            Nexus AI is connected to your data sources. Ask anything about your operational data and get instant answers with citations.
                        </p>
                                ...
                                <div className={`flex flex-col max-w-[80%] ${msg.role === "user" ? "items-end" : "items-start"}`}>
                                    <div className={`
                                        ...
                                        ${msg.role === "user" ? "bg-primary text-primary-foreground rounded-tr-sm" :
                                            msg.isError ? "bg-destructive/10 text-destructive rounded-tl-sm" :
                                    ...
                                    {msg.sources && msg.sources.length > 0 && (
                                        <div className="mt-2 text-xs text-muted-foreground flex flex-wrap gap-2">
                                            ...
                                            {msg.sources.map((s, i) => (
                                                <span key={i} className="flex items-center gap-1 bg-muted px-2 py-0.5 rounded-md border">
                ...
                <div className="p-4 border-t bg-card">
                    <form onSubmit={(e) => { e.preventDefault(); handleSend(); }} className="relative max-w-4xl mx-auto flex gap-2">
                        <Input
